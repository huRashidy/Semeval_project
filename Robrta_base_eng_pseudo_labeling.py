...
def print_f1_scores(true_labels, pred_labels, class_names):
    """
    Computes and plots the F1-score for each label in a multi-label classification problem.

    Args:
        true_labels (numpy.ndarray): Ground truth labels (binary multi-hot encoding).
        pred_labels (numpy.ndarray): Predicted labels (binary multi-hot encoding).
...
save_path = "/data/horse/ws/huel099f-huel099f-Semeval/hussin_code/saved_model/Roberta_base_pseudo_labeling"
...
with torch.no_grad():
    for batch in unlabeled_loader:
        ...
        pseudo_labels.extend((preds > 0.9).astype(int))  # Stricter threshold (0.7)
